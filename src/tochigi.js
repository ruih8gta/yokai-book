document.addEventListener('DOMContentLoaded', () => {
    const mapContainer = document.getElementById('tochigi-map-container');
    const detail = document.getElementById('tochigi-detail');
    const introEl = document.getElementById('tochigi-intro');
    const SVGNS = 'http://www.w3.org/2000/svg';
    let legends = [];

    Promise.all([
        fetch('maps/tochigi.svg').then(r => r.text()),
        fetch('tochigi.json').then(r => r.json())
    ]).then(([svgText, data]) => {
        ...
        console.error('栃木マップの読み込みに失敗しました:', err);
